...
// Ensure deterministic workspace resolution for path normalization
const path = require('path');
// Fixed: Removed hardcoded Windows path, using relative path from current directory
...
app.use(validateInput); // Add input validation
...
app.get("/health", (req, res) => {
  res.json({ status: "ok", pcm: mesh?.initialized || false });
});

// === Rate limiting middleware (improved version with cleanup) ===
// Replace the simple Map-based rate limiter with a proper solution
// Note: We're now using express-rate-limit instead of custom implementation
...
// ==== Enhanced main endpoint with improved security and validation ===
app.post("/free-coding-agent/run", async (req, res) => {
  try {
    ...
    // Validate input - already done in middleware but double checking
    ...
    const result = await mesh.think(input, { ...options, useTriad: true });
...
app.get("/metrics", async (req, res) => {
  try {
    const logPath = "/back/logs/metrics.log";
    if (!require("fs").existsSync(logPath)) {
      return res.status(200).send("# No metrics recorded yet");
    }

    const logContent = require("fs").readFileSync(logPath, "utf8");
    const lines = logContent.split("\n").filter(l => l.trim());

    const prometheusLines = lines.map(line => {
      const data = JSON.parse(line);
      return `# HELP free_coding_agent_requests_total Total requests
# TYPE free_coding_agent_requests_total counter
free_coding_agent_requests_total${data.request_id ? `.${data.request_id.slice(-5)}` : ""} ${data.request_id ? "1" : "0"}
# HELP free_coding_agent_requests_by_status Requests by HTTP status
# TYPE free_coding_agent_requests_by_status counter
free_coding_agent_requests_by_status${data.status} ${data.request_id ? "1" : "0"}
# HELP free_coding_agent_latency_ms Request latency in ms
# TYPE free_coding_agent_latency_ms histogram
free_coding_agent_latency_ms_bucket{le="+"Inf"} ${data.latency_ms ? String(data.latency_ms) : "0"}
      `;
    }).join("\n");

    res.set("Content-Type", "text/plain");
    res.send(prometheusLines);
  } catch (err) {
...
initMesh().then(() => {
  app.listen(PORT, () => console.log("PCM API running on port " + PORT));
}).catch(err => {
