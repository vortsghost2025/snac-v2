...
Blackwell optimized inference acceleration
"""
...
def load_cuda_kernels():
    """Load compiled CUDA kernel library"""
    global KERNEL_SO, CUDA_AVAILABLE
    kernel_paths = [
        Path(__file__).parent / "inference_kernel.so",
        Path(__file__).parent / "build" / "inference_kernel.so",
        Path("/usr/local/cuda/lib64/libreference.so"),
        Path("inference_kernel.so")
    ]

    for path in kernel_paths:
        try:
            # Ensure Path object and existence check works for absolute and relative paths
            p = Path(path)
            if not p.exists():
                continue

            log.info(f"Loading CUDA kernels from {p}")
            try:
                KERNEL_SO = ctypes.CDLL(str(p))
            except OSError as e:
                log.warning(f"Failed to load shared library {p}: {e}")
                continue

            # Validate that required kernel symbols exist
            required_symbols = [
                "cuda_embed_lookup",
                "cuda_layer_norm",
                "cuda_softmax",
            ...
            missing = [s for s in required_symbols if not hasattr(KERNEL_SO, s)]
            if missing:
                log.warning(f"Library {p} missing required functions: {missing}")
                KERNEL_SO = None
                continue
            ...
            log.warning(f"Unexpected error while loading {path}: {e}")

    log.warning("CUDA kernel library not found or invalid. GPU acceleration disabled.")
    return False


def cuda_embed_lookup(token_ids, embedding_table, batch_size, seq_len, vocab_size, embed_dim):
    """
    ...
    Returns:
        embeddings: np.array [batch_size, seq_len, embed_dim] float32
    """
    ...
    try:
        ...
        # Prepare GPU memory pointers
        token_ids_d = token_ids.astype(np.int32).ctypes.data_as(ctypes.POINTER(ctypes.c_int))
        embed_table_d = embedding_table.astype(np.float32).ctypes.data_as(ctypes.POINTER(ctypes.c_float))
        embeddings_d = embeddings.ctypes.data_as(ctypes.POINTER(ctypes.c_float))
        
        # Call kernel
        result = KERNEL_SO.cuda_embed_lookup(
            token_ids_d,
            embed_table_d,
            embeddings_d,
...
def cuda_layer_norm(input_data, weight, bias, eps=1e-5):
    ...
    Returns:
        output: np.array [N, hidden_size] float32
    """
    if not CUDA_AVAILABLE:
        return None
    ...
    try:
        ...
        input_d = input_data.astype(np.float32).ctypes.data_as(ctypes.POINTER(ctypes.c_float))
        weight_d = weight.astype(np.float32).ctypes.data_as(ctypes.POINTER(ctypes.c_float))
        ...
        result = KERNEL_SO.cuda_layer_norm(
...
def cuda_softmax(scores):
    ...
    Returns:
        softmax_scores: np.array [batch_size * seq_len, seq_len] float32
    """
    if not CUDA_AVAILABLE:
        return None
