...
class Agent {
  constructor(options = {}) {
    ...
    this.capacity = { maxConcurrent: options.maxConcurrent || 1, currentLoad: 0, maxMemoryMB: options.maxMemoryMB || 100, priority: options.priority ?? 0.5 };
    this.context = { caps: [], threads: [], maxCaps: options.maxCaps || 20 };
    this.inbox = [];
    this.outbox = [];
  ...
  async processAsCritic() {
    const { input } = this.currentTask;
    if (this.processor) {
      const response = await this.processor.process({ input, context: this.context.caps.map(c => c.content).join('\n---\n'), role: 'critic', instruction: 'Analyze for weaknesses.' });
  ...
  extractIssues(text) {
    const issues = [];
    const m = text.match(/\b(problem|issue|concern|risk|weakness|gap)\b[^.]*\./gi);
    if (m) issues.push(...m);
    return issues;
  }
