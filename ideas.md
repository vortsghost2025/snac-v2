...
AliceLJY
AliceLJY
Merge pull request #26 from AliceLJY/feat/shared-memory-scope
10668b6
 · 
3 minutes ago
.agents/skills
	
Add installation and prerequisites section to clawteam skill
...
feat: add CLAWTEAM_MEMORY_SCOPE env var and shared memory prompt hint
	
13 hours ago
docs
	
docs: add implementation plan for per-agent model assignment
	
2 days ago
scripts
	
fix: configure exec approvals in install script to prevent permission…
	
4 days ago
skills
	
docs: require proactive mid-progress updates and worktree merge
	
4 days ago
tests
	
feat: merge upstream bug fixes and improvements from HKUDS/ClawTeam
	
4 days ago
.gitignore
	
feat: sync strategy-room template and .gitignore from upstream
	
4 days ago
LICENSE
	
Release v0.1.1: add LICENSE, fix packaging, publish skills
	
5 days ago
README.md
	
docs: add per-agent model assignment preview section to README
...
Upstream Quick Start License

Python Agents Transport Version

    Fork of HKUDS/ClawTeam with deep OpenClaw integration: default openclaw agent, per-agent session isolation, exec approval auto-config, and production-hardened spawn backends. All upstream fixes are synced.
...
OpenClaw 	clawteam spawn tmux openclaw --team ... 	Default
...
Team Templates
    ...
    Per-agent model assignment (preview): assign different models to different roles — see below
...
Workspace, Plan, Lifecycle, Config

Per-Agent Model Assignment (Preview)

    Branch: feat/per-agent-model-assignment
...
See issue #1 for the full feature request and discussion.
Roadmap
Version 	What 	Status
...
youtu.be/aZT9d8qrirY
Topics
swarm swarms swarm-intelligence ai-agents clawdbot openclaw openclaw-skills openclaw-extension openclaw-plugin
Resources
Readme
License
MIT license
